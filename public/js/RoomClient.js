...
class RoomClient {
    ...
    async startLocalMedia() {
        ...
        try {
            // Extract meeting ID from room_id
            const meetingId = this.room_id;
            console.log('Calling API to start meeting with ID:', meetingId);
            
            // Make API call to start the meeting
            const response = await fetch(`/api/${meetingId}/start`, {
                method: 'PATCH',
                headers: {
            ...
            if (!response.ok) {
                console.error('Failed to start meeting:', await response.text());
            } else {
    ...
    handleMediaRecorderData(evt) {
        ...
        if (evt.data && evt.data.size > 0) {
            ...
            rc.recording.recSyncServerRecording ? recordedBlobs.push(evt.data) : recordedBlobs.push(evt.data);
        }
    }
